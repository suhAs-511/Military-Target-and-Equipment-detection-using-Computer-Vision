...
def upload():
    global ufp
    return render_template('index.html', filename=ufp)
...
def predict_img():
    if request.method == "POST":
        if 'file' in request.files:
            ...
            if file_extension == 'jpg':
                process = Popen(["python", "detect.py", '--source', filepath, "--weights","fighter.pt"], shell=True)
                process.wait()
            elif file_extension == 'mp4':
                process = Popen(["python", "detect.py", '--source', filepath, "--weights","fighter.pt"], shell=True)
                process.communicate()
...
if __name__ == "__main__":
    ...
    model = torch.hub.load('.', 'custom','fighter.pt', source='local')
    model.eval()
